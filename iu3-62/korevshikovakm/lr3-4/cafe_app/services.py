...
from .repositories import InMemoryOrderRepository, MenuRepository
@dataclass
class MenuService: #работа с меню
    repo: MenuRepository #репозиторий меню (in-memory или gRPC к сервису B)
    ...
    def create_item(self, category_id: int, name: str, price_text: str):
        # Проверка цены и каталога — в сервисе B (через gRPC), не на A.
        return self.repo.add_item(category_id=category_id, name=name, price_text=price_text)
...
@dataclass
class OrderService:
    menu_repo: MenuRepository #чтобы получать блюда (через справочник B)
