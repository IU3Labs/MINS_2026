...
class OrderService:
    ...
    def start_order(self, table_number: int): #открывает новый заказ на указанный стол
        return self.order_repo.create_order(table_number=table_number) #репозиторий проверяет номер стола, проверяет нет ли уже открытого заказа, создает ордер)
...
class CheckoutService: #оплата и чек
    ...
    def checkout(self, order_id: int, payment_method_key: str) -> str: #order_id какой заказ оплачиваем, payment_method_key чем оплачиваем
        ...
        total = order.total_amount() #считается общая сумма
        payment = processor.pay(total) #выбранный обработчик оплаты получает сумму и проводит оплату
        ...
        return self.receipt_service.generate_receipt_text(order, payment) #возвращает готовый чек
