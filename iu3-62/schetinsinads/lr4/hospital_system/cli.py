...
class CliApp:
    ...
    def _add_ward(self) -> None:
        ...
        is_icu = input("Реанимация? (y/n): ").strip().lower() in {"y", "yes", "д", "да"}
    ...
    def _list_doctors(self) -> None:
        ...
        for d in rows:
            print(f"{d.id} | {d.full_name} | {d.specialty} | {d.phone}")
    ...
    def _schedule_appointment(self) -> None:
        ...
        duration = ensure_positive_int(input("Длительность: "), "Длительность")
    ...
    def _list_appointments(self) -> None:
        ...
        for a in sorted(rows, key=lambda x: x.starts_at):
            patient = self._d.reference_client.get_patient_name(trace_id, a.patient_id)
            doctor = self._d.reference_client.get_doctor_name(trace_id, a.doctor_id)
            ward = self._d.reference_client.get_ward_number(trace_id, a.ward_id) if a.ward_id else "-"
            print(f"{a.id} | {patient} | {doctor} | {a.starts_at} | {a.duration_min} | {ward}")

    def _add_diagnosis(self) -> None:
        patient_id = input("Patient ID: ").strip()
        doctor_id = input("Doctor ID: ").strip()
        diagnosed_at = parse_datetime(input("Дата/время диагноза: "))
    ...
    def _list_diagnoses_by_patient(self) -> None:
        ...
        if not rows:
            ...
            return
        trace_id = uuid4().hex[:8]
        patient = self._d.reference_client.get_patient_name(trace_id, patient_id)
        print(f"\nИстория диагнозов пациента: {patient}")
        for d in rows:
            doctor = self._d.reference_client.get_doctor_name(trace_id, d.doctor_id)
            print(f"{d.id} | {d.diagnosed_at} | {doctor} | {d.diagnosis} | {d.notes}")

    def _issue_prescription(self) -> None:
        patient_id = input("Patient ID: ").strip()
        doctor_id = input("Doctor ID: ").strip()
        prescribed_at = parse_datetime(input("Дата/время рецепта: "))
        medicine = input("Лекарство: ")
        dosage = input("Дозировка: ")
        instructions = input("Инструкция: ")
        days = ensure_positive_int(input("Срок действия: "), "Срок действия")
        p = self._d.prescriptions.issue(patient_id, doctor_id, prescribed_at, medicine, dosage, instructions, days)
    ...
    def _list_prescriptions_by_patient(self) -> None:
        ...
        for p in rows:
            doctor = self._d.reference_client.get_doctor_name(trace_id, p.doctor_id)
            print(f"{p.id} | {p.prescribed_at} | {doctor} | {p.medicine} | {p.dosage}")

    def _admit_patient(self) -> None:
        patient_id = input("Patient ID: ").strip()
        ward_id = input("Ward ID: ").strip()
        admitted_at = parse_datetime(input("Дата/время поступления: "))
    ...
    def _discharge_patient(self) -> None:
        ...
        discharged_at = parse_datetime(input("Дата/время выписки: "))
    ...
    def _list_admissions_by_patient(self) -> None:
        ...
        for a in rows:
            ward = self._d.reference_client.get_ward_number(trace_id, a.ward_id)
            print(f"{a.id} | {ward} | {a.admitted_at} | {a.discharged_at}")
    ...
    def _show_reminders(self) -> None:
        ...
        for appt, left in rows:
            patient = self._d.reference_client.get_patient_name(trace_id, appt.patient_id)
            doctor = self._d.reference_client.get_doctor_name(trace_id, appt.doctor_id)
    ...
    def _calculate_treatment_cost(self) -> None:
        ...
        total = self._d.treatment_cost.quick_calculate(days, appointments, diagnoses, prescriptions, icu)
