...
public class TicketCommandServiceImpl implements TicketCommandService {
    ...
    public Result<UUID> buyTicket(UUID screeningId, Seat seat, int categoryIndex) {
        try {
            ...
            pricingService.setStrategy(pricingService.getStrategies().get(categoryIndex).getStrategyId());
            pricingService.calculatePrice(ticket);
    ...
    public Result<Void> cancelTicket(UUID ticketId) {
        try {
            ...
            if (!ticket.canCancel()) {
                throw new ValidationException("Нельзя отменить билет со статусом " + ticket.getStatusName());
            }
