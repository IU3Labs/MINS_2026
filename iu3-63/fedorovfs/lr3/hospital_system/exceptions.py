...
class BusinessLogicException(HospitalException):
    """Бизнес-логика нарушена (для сервисов)"""
...
class InvalidPatientStateException(BusinessLogicException):
    """Операция недопустима в текущем состоянии пациента."""

    def __init__(self, message: str):
        super().__init__(message, operation="смена состояния пациента")
        self.error_code = 3005


class EmailNotificationException(HospitalException):
    """Базовая ошибка канала уведомлений по e-mail (имитация SMTP / шлюза)."""
...
class EmailInvalidRecipientException(EmailNotificationException):
    """Нельзя сформировать или проверить адрес получателя."""

    def __init__(self, reason: str, recipient_hint: Optional[str] = None):
        d: Dict[str, Any] = {"reason": reason}
        if recipient_hint is not None:
            d["recipient_hint"] = recipient_hint
        super().__init__(
            f"Некорректный получатель письма: {reason}",
            error_code=6001,
            details=d,
        )


class EmailDeliveryFailedException(EmailNotificationException):
    """Сбой доставки (сеть, шлюз, политика) при отправке письма."""

    def __init__(self, reason: str, technical_detail: Optional[str] = None):
        d: Dict[str, Any] = {"reason": reason}
        if technical_detail:
            d["technical_detail"] = technical_detail
        super().__init__(
            f"Не удалось отправить e-mail: {reason}",
            error_code=6002,
            details=d,
        )
