...
class HospitalSystemUI:
    def __init__(
        ...
        self.commands = {
            ...
            "16": self._change_patient_status
        }
    ...
    def _quick_cost_estimate(self) -> None:
        try:
            ...
            print(f"Cтоимость лечения для пациента '{patient.name}': {cost} руб.")
        except ValueError:
    ...
    def _change_patient_status(self) -> None:
        """Смена статуса в обход бизнес-логики
        Swallowing Exceptions и нарушение инкапсуляции.
        """
        print("\nСМЕНА СТАТУСА ПАЦИЕНТА")
        p_id_str = input("Введите ID пациента: ").strip()
        new_state_key = input(
            "Введите новый ключ статуса (not_hospitalized, scheduled, hospitalized, discharged): ").strip()
        try:
            p_id = int(p_id_str)
            # спагетти код / хардкодед dependencies:
            from patient_states import patient_state_from_key  # импорт прямо в теле функции
            new_state = patient_state_from_key(new_state_key)

            patient = self.patient_service.patient_repo._storage[p_id]  # обращаемся в приватному мвойству
            patient.state = new_state
            print(f"Статус пациента {patient.name} изменен на: {new_state.label}")
        except Exception:
    ...
    def _print_menu(self) -> None:
        ...
        print("15. Каолькулятор стоимости лечения")
        print("16. Изменить статус пацента")
        print("0. Выход")
