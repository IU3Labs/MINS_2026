...
from typing import Any  # Добавлено для поддержки Any в типах сервисов
...
# Импортируем только те сервисы, которые остались в Сервисе А
from services import AppointmentService, PatientService

from quick_billing import get_quick_cost_estimate
...
class HospitalSystemUI:
    ...
    def _complete_appointment(self) -> None:
        try:
            ...
            print("🔢 Ошибка: введите корректный ID приёма.")
    ...
    def _show_all_patients(self) -> None:
        ...
        for p in patients:
            ...
            # Метод get_ward в микросервисной версии может возвращать None или работать иначе,
            # так как ward_service в Сервисе А теперь равен None
            print(
                f"ID: {p.id} | Имя: {p.name} | Возраст: {p.age} | Статус: {p.state.label} | "
                f"ID Палаты: {p.ward_id if p.ward_id else '—'} | История: {history}"
            )

    # Примечание: методы ниже (6-11) будут вызывать ошибку, так как doctor/ward сервисы теперь None.
    # Это ожидаемое поведение при разделении на микросервисы.
    ...
    def _show_all_wards(self) -> None:
        ...
        if not wards:
            ...
            return
        for w in wards:
            print(f"ID: {w.id}  Номер: {w.number}  Вместимость: {w.capacity}")
    ...
    def _edit_doctor(self) -> None:
        ...
        name = input("Введите новое имя врача: ").strip()
        specialization = input("Введите новую специализацию: ").strip()
        self.doctor_service.update_doctor(doctor_id, name=name or None, specialization=specialization or None)
    ...
    def _show_all_appointments(self) -> None:
        ...
        for a in appointments:
            print(f"ID: {a.id}  Пациент: {a.patient_id}  Врач: {a.doctor_id} | Диагноз: {a.diagnosis}")
    ...
    def _quick_cost_estimate(self) -> None:
        try:
            patient_id = int(input("Введите ID пациента для оценки стоимости: ").strip())
            patient = self.patient_service.patient_repo.get_by_id(patient_id)
            if not patient:
                print("❌ Пациент не найден.")
                return
            cost = get_quick_cost_estimate(patient)
            print(f"💰 Стоимость лечения для пациента '{patient.name}': {cost} руб.")
        except ValueError:
            print("🔢 Ошибка: введите корректный ID.")

    def _export_data(self) -> None:
        print("Что экспортировать: 1 — пациенты, 2 — врачи (N/A), 3 — палаты (N/A), 4 — приёмы")
        choice = input("Выбор: ").strip()
        filename = input("Имя файла (без расширения): ").strip() or "export_result"
        ...
        if choice == "1":
            ...
            print("❌ Этот тип данных недоступен в текущем сервисе.")
            return
        save_export(data, path)
        print(f"✅ Файл сохранён: {path}")

    def _print_menu(self) -> None:
        print("\nИнформационная система больницы (Service A: Core)")
        print("1. Регистрация пациента")
        print("2. Госпитализировать пациента (gRPC -> Service B)")
        print("3. Записать пациента на приём (gRPC -> Service B)")
        print("4. Завершить приём пациента")
        print("5. Показать всех пациентов")
        print("12. Показать список приёмов")
        print("13. Выписать пациента")
        print("14. Экспорт данных")
        print("15. Калькулятор стоимости лечения (Magic Numbers)")
        print("0. Выход")
