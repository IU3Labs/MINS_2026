...
class PatientService:
    """Сервис работы с пациентами (Service A)"""

    def __init__(
        self,
        patient_repo: Repository,
        reference_client,  # gRPC клиент вместо локального ward_repo
    ...
    def admit_to_ward(self, patient_id: int, ward_id: int) -> None:
        ...
        if isinstance(patient.state, HospitalizedState) or patient.ward_id is not None:
        ...
        # запрашиваем данные о палате из Service B
        ward_response = self.reference_client.get_ward_info(ward_id)
        if not ward_response.exists:
            raise WardNotFoundException(ward_id=ward_id)

        # считаем заполненность палаты
        current_ward_weight = sum(1 for p in self.patient_repo.get_all() if p.ward_id == ward_id)

        if current_ward_weight >= ward_response.capacity:
            raise WardFullException(
                ward_number=ward_response.number,
                capacity=ward_response.capacity,
                current_count=current_ward_weight,
            )
        ...
        self._notify(
            NotificationEvent(
                ...
                message=f"Пациент {patient.name} размещён в палате №{ward_response.number}",
                payload={
                    "patient_id": patient.id,
                    "patient_name": patient.name,
                    "ward_id": ward_id,
    ...
    def discharge_patient(self, patient_id: int) -> None:
        ...
        if not isinstance(patient.state, HospitalizedState) or patient.ward_id is None:
...
class AppointmentService:
    """Логика работы с приёмами и диагнозами (Service A)"""
    ...
    def schedule_appointment(self, patient_id: int, doctor_id: int, dt: datetime) -> Appointment:
        ...
        # проверяем наличие врача
        doctor_response = self.reference_client.get_doctor_info(doctor_id)
        if not doctor_response.exists:
...
class DoctorService:
    """Логика работы с врачами (Используется теперь в Service B)"""
...
class WardService:
    """Логика работы с палатами (Используется теперь в Service B)"""
