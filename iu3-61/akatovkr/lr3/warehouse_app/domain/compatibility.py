...
from warehouse_app.exceptions import CompatibilityError, UserInputError


# интерфейс одного правила
class ICompatibilityRule(ABC):
    # задаёт две вещи
    # можно ли хранить вместе
    @abstractmethod
    def is_compatible(self, first: Product, second: Product) -> bool:
        raise NotImplementedError
    # как это правило называется человеческим текстом
    @abstractmethod
    def description(self) -> str:
        raise NotImplementedError
# шаблон для любого правила
    ...
    def get(self, code: str) -> ICompatibilityRuleFactory:
        ...
        if factory is None:
            ...
            raise UserInputError(
                f"Неизвестный набор правил '{code}'. Доступно: {available}"
            )
        return factory


def build_default_rule_factory_registry() -> CompatibilityRuleFactoryRegistry:
    return CompatibilityRuleFactoryRegistry(
        [
            SanPinRuleFactory(),
            GostRuleFactory(),
            InternalWarehouseRuleFactory(),
        ]
    )
