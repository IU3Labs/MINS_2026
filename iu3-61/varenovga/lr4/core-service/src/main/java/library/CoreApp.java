...
public class CoreApp {
    public static void main(String[] args) {
        ...
        inputHandler.log("=== БИБЛИОТЕЧНАЯ СИСТЕМА (SOLID + gRPC) ===");

        while (running) {
            menuRenderer.printMenu();
            String choice = inputHandler.readChoice(scanner);
            try {
                MenuAction action = MenuAction.fromCode(choice).orElseThrow(() -> new IllegalArgumentException("Неверный выбор"));
                switch (action) {
                    ...
                    case EXIT -> {
                        ...
                        inputHandler.log("Observer сработал раз: " + service.notifications.EmailNotificationObserver.getNotificationCounter());
                    }
                }
            } catch (IllegalArgumentException | LibrarySystemException e) {
