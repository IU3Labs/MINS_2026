...
public class OrderServiceImpl implements OrderService {
    ...
    public Order createOrder(String from, String to, double weight) {
        System.out.println("[OrderService] createOrder - НАЧАЛО");

        if (weight <= 0 || weight > 1000) {
            throw new InvalidWeightException(weight);
        }

        System.out.println("[OrderService] Создаю объект Order...");
        Order order = new Order(UUID.randomUUID().toString(), from, to, weight, OrderStatus.NEW, null, 0, new NewOrderState(), new ArrayList<>());

        System.out.println("[OrderService] Вычисляю цену (1-й раз)...");
        order.setPrice(currentStrategy.calculate(order));

        System.out.println("[OrderService] Прикрепляю observer...");
        order.attachObserver(fileUpdateObserver);

        System.out.println("[OrderService] Вычисляю цену (2-й раз)...");
        order.setPrice(currentStrategy.calculate(order));

        System.out.println("[OrderService] Сохраняю в репозиторий...");
        orderRepository.save(order);

        System.out.println("[OrderService] createOrder - ЗАВЕРШЕНО");
        return order;
    }
